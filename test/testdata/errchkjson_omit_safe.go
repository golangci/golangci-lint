// args: -Eerrchkjson
// config_path: testdata/configs/errchkjson_omit_safe.yml
package testdata
...
func JSONMarshalSaveTypesWithNoSafe() {
	...
	_, err = json.MarshalIndent(nil, "", " ") // nil is safe, but omit-safe is set
	_ = err
	...
	_, err = json.Marshal(basicStruct) // struct containing only safe basic types is safe, but omit-safe is set
	_ = err
	...
	_, err = json.Marshal(ptrStruct) // struct containing pointer to only safe basic types is safe, but omit-safe is set
	_ = err
	...
	_, err = json.Marshal(mapStrStr) // map[string]string is safe, but omit-safe is set
	_ = err

	var mapStrInt map[string]int
	_, _ = json.Marshal(mapStrInt)   // ERROR "Error return value of `encoding/json.Marshal` is not checked"
	_, err = json.Marshal(mapStrInt) // map[string]int is safe, but omit-safe is set
	_ = err
	...
	_, err = json.Marshal(outerStruct) // struct with only safe types is safe, but omit-safe is set
	_ = err
}
