...
The easiest way to use `golangci-lint` in CI for GitLab projects, is by using a Docker image in the pipeline job.
...
golangci-lint:
  stage: lint
  image: golangci/golangci-lint:{.LatestVersion}
  script:
    golangci-lint run -v
```
