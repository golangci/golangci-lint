...
GolangCI-Lint stores his cache into the [default user cache directory](https://golang.org/pkg/os/#UserCacheDir).

You can override the default cache directory with the environment variable `GOLANGCI_LINT_CACHE`, the path must be an absolute path.
