...
1. Implement functional tests for the linter:
    - Add one file into directory [`test/testdata`](https://github.com/anduril/golangci-lint/tree/master/test/testdata).
...
3. Add the new struct for the linter (which you've implemented in `pkg/golinters/{yourlintername}.go`) to the
   list of all supported linters in [`pkg/lint/lintersdb/manager.go`](https://github.com/anduril/golangci-lint/blob/master/pkg/lint/lintersdb/manager.go)
...
4. Find out what options do you need to configure for the linter.
   For example, `nakedret` has only 1 option: [`max-func-lines`](https://github.com/anduril/golangci-lint/blob/master/.golangci.reference.yml).
   Choose default values to not being annoying for users of golangci-lint. Add configuration options to:
    - [.golangci.reference.yml](https://github.com/anduril/golangci-lint/blob/master/.golangci.reference.yml) - the example of a configuration file.
      You can also add them to [.golangci.yml](https://github.com/anduril/golangci-lint/blob/master/.golangci.yml)
      if you think that this project needs not default values.
    - [config struct](https://github.com/anduril/golangci-lint/blob/master/pkg/config/config.go) -
      don't forget about `mapstructure` tag for proper configuration files parsing by [pflag](https://github.com/spf13/pflag).
5. Take a look at the example of [Pull Request with new linter support](https://github.com/anduril/golangci-lint/pulls?q=is%3Apr+is%3Amerged+label%3A%22linter%3A+new%22).
...
1. If the project you want to lint does not have one already, copy the [.golangci.yml](https://github.com/anduril/golangci-lint/blob/master/.golangci.yml) to the root directory.
