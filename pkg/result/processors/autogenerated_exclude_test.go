...
func TestAutogeneratedExclude_isGeneratedFileLax(t *testing.T) {
	all := `
...
`
	...
	for _, ngc := range notGeneratedCases {
		isGenerated := p.isGeneratedFileLax(ngc)
		assert.False(t, isGenerated)
	}
}

func Test_getPackageComments(t *testing.T) {
	testCases := []struct {
		fpath string
		doc   string
	}{
...
func Test_getPackageComments_fileWithLongLine(t *testing.T) {
	fpath := filepath.Join("testdata", "autogen_exclude_long_line.go")
	_, err := getPackageComments(fpath)
